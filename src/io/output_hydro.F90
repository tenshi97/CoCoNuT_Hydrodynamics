...
!  use param_rt , only : p_ntr
  ...
  use tracer_cfc, only : tracer_num, n_ts, n_te, tracer_id, pos_tracer_r, pos_tracer_theta, tracer_proc,tracer_total
  implicit none
! LOCAL variables that are not in modules
...
!----------------------------------------------------------------------
     ...
     call writeq(hfile,"try",pos_tracer_theta,"Tracer Particles Y Coordinate(Spherical-Polar)","cm", &
       "index",(/n_ts,n_te/),(/1,tracer_total/))
